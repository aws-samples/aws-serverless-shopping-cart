...
def lambda_handler(event, context):
    ...
    for record in records:
        ...
        if old_image_ddb:
            old_image = dynamodb_to_python(record['dynamodb'].get('OldImage'))  # Won't exist in case event is INSERT
        else:
            old_image = {}

        # We want to record the quantity change the change made to the db rather than absolute values
        if keys['sk'].startswith('product#'):
            quantity_change_counter.update({keys['sk']: new_image.get('quantity', 0) - old_image.get('quantity', 0)})
